...
const rootReducer = (state = initState, action) => {
  if (action.type === ActionTypes.FETCH_COMPS_DATA_SUCCESS) {
    return {
      ...state,
      competitions: action.payload.data,
    };
  } else if (action.type === ActionTypes.SET_ACTIVE_COMP) {
    return {
      ...state,
  ...
  } else if (action.type === ActionTypes.SET_ACTIVE_SEASON) {
    return {
      ...state,
      activeSeason: action.payload,
      breadcrumb: [...state.breadcrumb, action.payload.Name],
    };
  } else if (action.type === ActionTypes.SET_ACTIVE_SEASON_TEAMS) {
    return {
      ...state,
      seasonTeams: action.payload,
    };
  } else if (action.type === ActionTypes.SET_ACTIVE_TEAM) {
    return {
      ...state,
      activeTeam: action.payload,
      breadcrumb: [...state.breadcrumb, action.payload.TeamName],
    };
  } else if (action.type === ActionTypes.SET_ACTIVE_SQUAD) {
    return {
      ...state,
      squad: action.payload,
    };
  } else if (action.type === ActionTypes.SET_BREADCRUMB) {
    return {
      ...state,
      breadcrumb: [...state.breadcrumb, action.payload],
    };
  } else if (action.type === ActionTypes.REMOVE_FROM_BREADCRUMB) {
    return {
      ...state,
      breadcrumb: state.breadcrumb.slice(0, action.payload),
    };
  } else if (action.type === ActionTypes.RESET_BREADCRUMB) {
    return {
      ...state,
      breadcrumb: ["Home"],
    };
